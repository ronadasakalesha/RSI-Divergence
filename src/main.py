...
def get_next_candle_close_time():
    ...
    if next_min >= 60:
        next_min %= 60
        return now.replace(minute=next_min, second=0, microsecond=0) + timedelta(hours=1)
...
def wait_for_candle_close():
    ...
    if wait_seconds > 300:
        logger.warning("[WAIT] Abnormally large wait skipped")
        return False

    if wait_seconds > 0:
        logger.info(f"[WAIT] Candle closes at {next_close.strftime('%H:%M:%S')}")
...
def main():
    ...
    while True:
        try:
            ...
            logger.info("[FETCH] Fetching candles...")
            ...
            if df is None or len(df) == 0:
                logger.warning("[WARNING] No data received")
                continue

            # ===== UTC → IST (MANDATORY FIX) =====
            ...
            if signal:
                ...
                if ENABLE_TELEGRAM_ALERTS:
                    ...
                    msg = (
                        f"{emoji} <b>{signal['type']} DIVERGENCE</b>\n\n"
                        f"<b>Symbol:</b> {SYMBOL}\n"
                        f"<b>TF:</b> {TIMEFRAME}\n"
